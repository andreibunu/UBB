...
public class FA {
    ...
    public FA(String fileName) {
        ...
        System.out.println(checkIfAccepted());
    ...
    public boolean checkIfAccepted(){
        ...
        toGo.add(this.initialState);
        boolean change = true;
        ArrayList<String> lastTogo = new ArrayList<>();
        while(!toGo.isEmpty() && change){
            ArrayList<String> newToGo = new ArrayList<>();
            change = false;
            for(int i=0;i<toGo.size();i++) {
                for (Pair<String, String> pair : this.transitions.keySet()) {
                    if (pair.getKey().equals(toGo.get(i)) && !pair.getKey().equals(pair.getValue())){
                        newToGo.add(pair.getValue());
                        change = true;
                    }
                }
            }
        ...
        for (String go : lastTogo) {
            if (!this.finalState.contains(go)) {
                return false;
            }
        }
        return true;
